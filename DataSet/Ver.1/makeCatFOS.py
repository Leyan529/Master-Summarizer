...
D:	
activate tensorflow
python makeCatFOS.py
'''

glob('pickle/main_cat_data/%s.pickle' % (main_cat))

# reload a file to a variable
with open(glob('pickle/main_cat_data/%s.pickle' % (main_cat))[0], 'rb') as file:
	print(file.name)
...
Reviews = 0
fn = 'FOP-View/%s_keywords.txt' % (main_cat)
if not os.path.exists(fn):
	print("Start get Product keywords...")
	with open('FOP-View/Prod_%s.txt' % (main_cat), "w", encoding="utf-8") as f:
		...
		with tqdm(total=Products) as pbar:
			for asin, DATA_DICT in PROD_DICT.items():
				...
				for pair in REVIEW_ITEM_LIST:
					...
					pbar.set_description("%s Product : %s , keywords Processing ID %s" % (main_cat, asin, review_ID))
	...
	fn2 = 'FOP-View/%s_keywords2.txt' % (main_cat)
	...
	fn3 = 'FOP-View/%s_keywords3.txt' % (main_cat)
...
if not os.path.exists('XLSX/result'):
	os.makedirs('XLSX/result')
...
with tqdm(total=Reviews) as pbar:
	with open("FOP-View/%s_result.txt" % (main_cat), 'w', encoding="utf-8") as result:
		for asin, DATA_DICT in PROD_DICT.items():
			REVIEW_ITEM_LIST = DATA_DICT['REVIEW_ITEM_LIST']
			title = DATA_DICT['title']
			...
			if Reviews == len(PROD_DICT.items()): 
				pbar.update(1)
				pbar.set_description("%s Product : %s " % (main_cat, asin))
			result.write('************************************************************************' + "\n")
			for pair in REVIEW_ITEM_LIST:
				review = pair["review"]
				...
				if Reviews != len(PROD_DICT.items()): 
					pbar.update(1)
					pbar.set_description("%s Product : %s , FOP Processing ID %s  " % (main_cat, asin, review_ID))
				...
				key_fop_pair_row['main_cat'] = main_cat
...
if not os.path.exists('XLSX/main_cat'):
	os.makedirs('XLSX/main_cat')
...
# Key FOP Sentence DataSet 建立
csv_path = "XLSX/main_cat/%s_key_FOP_Sent.xlsx" % (main_cat)
...
csv_path = "XLSX/main_cat/%s_key.xlsx" % (main_cat)
# df.to_csv(csv_path) #默认dt是DataFrame的一个实例，参数解释如下
