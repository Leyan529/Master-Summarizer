...
class Beam(object):
    ...
    def advance(self, n_rem, prob_dist, step, converge):
        ...
        best_scores, best_scores_id = T.topk(input=scores, k=config.beam_size, dim=0, sorted=True)   #will be sorted in descending order of scores
        ...
        beams_order = best_scores_id.squeeze(1)/n_extended_vocab        #(beam,); sorted   # sort_beams
        ...
        self.log_scores = self.log_scores[beams_order]    # 排序過去的past log_scores使其與新計算的self.scores對應
        self.log_scores = T.cat([self.log_scores, self.scores], dim=1)           #(beam, t+1);
        ...
        # decay_prob = [(penalties / prob).item() for prob in sum_log_scores]
