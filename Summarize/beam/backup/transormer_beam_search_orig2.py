...
# def pad_to_batch_size(pad_tensor):    
#     pad_tensor = T.cat((pad_tensor,pad_tensor,pad_tensor,pad_tensor,pad_tensor),dim=0)
#     pad_tensor = pad_tensor[:config.batch_size]
#     return pad_tensor
...
def beam_search(config, batch, model, start_id, end_id, unk_id):
    ...
    #     beams = [Beam(start_id, end_id, unk_id, coverage) for i in range(batch_size)]   #For each example in batch, create Beam object
    # else:
    #     beams = [Beam(start_id, end_id, unk_id, coverage_t_0[0]) for i in range(batch_size)]   #For each example in batch, create Beam object
    ...
    for t in range(config.max_dec_steps):
        ...
        # if encoder_outputs.shape[0] != hyp_tokens.shape[0]:
        
        # print('batch_size : ',config.batch_size,'n_rem : ',n_rem)
        # print('hyp_tokens1',hyp_tokens.shape)
        # hyp_tokens = T.cat((hyp_tokens,hyp_tokens,hyp_tokens,hyp_tokens,hyp_tokens),dim=0)
        # print('hyp_tokens2',hyp_tokens.shape)
        # # print((encoder_outputs.shape[0]-n_rem+1))
        # print('encoder_outputs',encoder_outputs.shape)
        # hyp_tokens = hyp_tokens[:config.batch_size]
        # print('hyp_tokens3',hyp_tokens.shape)
        # print('step',t)
        # print('hyp_beams')
        # print([beam.get_current_state() for beam in beams if beam.done == False])
        ...
        # print('--------------------------------------------')
        if hyp_tokens.shape[0]!= config.batch_size:
            '''no more hyp_tokens -STOP'''
            break
        ...
        # if n_rem != pred.shape[0]: n_rem = pred.shape[0]
        # print('pred',pred.shape)
        # print((n_rem, config.beam_size, -1))
        # if n_rem != pred.shape[0]:
        #     pred = pred.view(pred.shape[0], config.beam_size, -1)
        # else:
        #     pred = pred.view(n_rem, config.beam_size, -1)                   #final_dist: rem, beam, n_extended_vocab
        pred = pred.view(n_rem, config.beam_size, -1)            
