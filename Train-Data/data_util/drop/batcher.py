...
class Example(object):
  ...
  # def __init__(self, review, summary_sentences,orign_review_text, vocab):
    ...
    start_decoding = vocab.word2id(data.START_DECODING) # start_decoding = 2
    stop_decoding = vocab.word2id(data.STOP_DECODING) # stop_decoding = 3
    ...
    if len(review_words) > config.max_enc_steps:
      review_words = review_words[:config.max_enc_steps] # 調小max_enc_steps
    ...
    key_words = [word for word in key_words if word in vocab._word_to_id.keys()] # 過濾不在vocabulary 的 keyword
    ...
    self.enc_key_input = [vocab.word2id(w) for w in key_words]  # list of keyword ids; NO UNK token
    ...
    self.enc_input_extend_vocab, self.review_oovs = data.review2ids(review_words, vocab)
    ...
    sum_ids_extend_vocab = data.summary2ids(summary_words, vocab, self.review_oovs)
...
class Batch(object):  # call by class Batcher (fill_batch_queue)
  ...
  def init_encoder_seq(self, example_list):
      ...
      self.enc_key_padding_mask = np.zeros((self.batch_size, max_enc_seq_len), dtype=np.float32)

      # Fill in the numpy arrays
      for i, ex in enumerate(example_list):
        self.enc_batch[i, :] = ex.enc_input[:]
        # print(i,ex.enc_input[:])
...
class Batcher(object):
  ...
  def fill_example_queue(self):
    ...
    while True:
      ...
      # summary_sentences = [sent.strip() for sent in data.summary2sents(summary)] # Use the <s> and </s> tags in summary to get a list of sentences.
  ...
  def text_generator(self, example_generator):
    while True:
      ...
      try:
        ...
        review_text = e.features.feature['review'].bytes_list.value[0]  # the review text was saved under the key 'review' in the data files
        # summary_text = b' <s> solid value performer </s> '
        summary_text = e.features.feature['summary'].bytes_list.value[0]  # the summary text was saved under the key 'summary' in the data files
...
#         tf.logging.error('---------------------------------------------------')
        try:
            keywords_text = e.features.feature[config.keywords].bytes_list.value[0]  # the keywords text was saved under the key 'keywords' in the data files
