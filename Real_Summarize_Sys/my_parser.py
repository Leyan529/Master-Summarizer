...
def parse_page(htmlpage):
    ...
    for block in blocks:
        ...
        lemm_reviewtext = lemm(reviewtext)
        if len(lemm_reviewtext.split(" ")) <= 40: continue
        POS_keys, DEP_keys, Noun_adj_keys = get_keys(lemm_reviewtext)
        row.append([rating, date, summary, reviewtext, lemm_reviewtext, binaryrating, POS_keys, DEP_keys, Noun_adj_keys])
