...
class Train(object):
    ...
    def train_batch_MLE(self, enc_out, enc_hidden, enc_padding_mask, ct_e, extra_zeros, enc_batch_extend_vocab, batch):
        ...
        :param ct_e: encoder context vector for time_step=0 (eq 5 in DEEP REINFORCED MODEL - https://arxiv.org/pdf/1705.04304.pdf)
        ...
        sum_temporal_srcs = None                                                                    #Used for intra-temporal attention (section 2.1 in DEEP REINFORCED MODEL - https://arxiv.org/pdf/1705.04304.pdf)
        for t in range(min(max_dec_len, config.max_dec_steps)):
            # use_gound_truth 為 model在訓練時會隨機決定下一個word要從previous token產生，
            # 或是從ground true token產生，用來避免exposure bias
    ...
    def train_one_batch(self, batch, iter):
        ...
        if self.opt.train_rl == "yes":                                                              #perform reinforcement learning training
            ...
            rl_loss = -(sample_reward - baseline_reward) * RL_log_probs                             #Self-critic policy gradient training (eq 15 in DEEP REINFORCED MODEL - https://arxiv.org/pdf/1705.04304.pdf)
