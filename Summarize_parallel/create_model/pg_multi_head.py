...
class encoder_attention(nn.Module):
    def __init__(self, h, d_model, d_emb, dropout=0.1):
        ...
        self.d_k = d_model // h
        self.h = h
        self.key_linear = nn.Linear((d_model+d_emb),d_model)
        self.linears = self.clones(nn.Linear(d_model, d_model), 4)
        self.attn = None
        self.dropout = nn.Dropout(p=dropout)

    def forward(self, query, key, value, mask=None, sum_temporal_srcs=None, sum_k_emb=None):
        "實現多頭注意力模型"
        if mask is not None:
        ...
        if config.key_attention: 
            query = T.cat([query, sum_k_emb], dim = 1) # [1, 1024 + 300]
            query = self.key_linear(query)
...
class MultiHead_attention(nn.Module):
    ...
    def forward(self, query, key, value, sum_k_emb=None):
        "實現多頭注意力模型"
        if config.key_attention:     
            query = T.cat([query, sum_k_emb], dim = 1) # [1, 1024 + 300]
            query = self.key_linear(query)
            pass
        if key is None:            
            ct_d = T.zeros(query.size()).cuda(query.device.index)
            prev_s = query.unsqueeze(1)               #batch_size, 1, hid_size
            return ct_d, prev_s, None
        else: 
            # key = query.unsqueeze(1) 
            # value = query.unsqueeze(1) 
            prev_s = key
            prev_s = T.cat([prev_s, query.unsqueeze(1)], dim=1)    #batch_size, t, hid_size  # 將目前計算的decoder state 合併到 previous decoder 
            nbatches = query.size(0)
            "第二步是將這一批次的數據進行變形 d_model => h x d_k"
            # orign_query = query
            query, key, value = \
            ...
            x = x.transpose(1, 2).contiguous().view(nbatches, -1, self.h * self.d_k)
            # result of concat : torch.Size([2, 1, 1024])
            ct_d = self.linears[-1](x).squeeze(1)            
            return ct_d, prev_s, self.attn.squeeze(2)
...
class Decoder(nn.Module):
    def __init__(self):
        ...
        self.enc_attention = encoder_attention(4, config.hidden_dim*2, config.emb_dim)
        # self.dec_attention = decoder_attention()
        self.dec_attention = MultiHead_attention(4, config.hidden_dim, config.emb_dim)
    ...
    def forward(self, x_t, s_t, enc_out, enc_padding_mask, ct_e, extra_zeros, enc_batch_extend_vocab, sum_temporal_srcs, prev_s, key_x, key_mask):
        ...
        ct_e, attn_dist, sum_temporal_srcs = self.enc_attention(st_hat, enc_out, enc_out, enc_padding_mask, sum_temporal_srcs, sum_k_emb)
        ...
        ct_d, prev_s, dec_attn = self.dec_attention(dec_h, prev_s, prev_s, sum_k_emb)
