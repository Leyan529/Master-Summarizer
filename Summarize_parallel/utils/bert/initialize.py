...
def loadCheckpoint(config, logger, load_model_path, model):    
    ...
    if not config.sep_optim:
        optim = checkpoint['optimizer'][0]
        saved_optimizer_state_dict = optim.optimizer.state_dict()
        optim.optimizer.load_state_dict(saved_optimizer_state_dict)
        optimizer = [optim]

    else:
        optim_bert = checkpoint['optimizer'][0]
        saved_bert_state_dict = optim_bert.optimizer.state_dict()
        optim_bert.optimizer.load_state_dict(saved_bert_state_dict)

        optim_dec = checkpoint['optimizer'][1]
        saved_dec_state_dict = optim_dec.optimizer.state_dict()
        optim_dec.optimizer.load_state_dict(saved_dec_state_dict)
