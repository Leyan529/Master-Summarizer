...
# df = df[(df.lemm_summary_len >= 4) ] # 過濾single word summary
# # df = df[(df.lemm_review_len <= 1000) ] # 過濾single word summary
# df = df[(df.lemm_review_len >= 10) ] # 過濾single word summary
...
if not os.path.exists(csv_path):
    with tqdm(total=len(df)) as pbar:
        ...
        for idx in range(len(df)):           
            ...
            # if summary.split(" ") / set(summary.split())
...
# df = df[(df.bert_summary_len >= 8) ] # 過濾single word summary
# df = df[(df.bert_summary_len <= 60) ] # 過濾single word summary
# df = df[(df.bert_review_len <= 1000) ] # 過濾single word summary
# df = df[(df.bert_review_len >= 20) ] # 過濾single word summary
# df = df[(df.cheat_num >= 4) ] # 過濾single word summary
...
if not os.path.exists("%s/Embedding/word2Vec/word2Vec.300d.txt"%(folder)):    
    w2vec = word2vec.Word2Vec(embedding_corpus, size=300, min_count=1,max_vocab_size=None,iter=100,
...
# Embedding/FastText
if not os.path.exists("%s/Embedding/FastText/FastText.300d.txt"%(folder)):
    fasttext = FastText(embedding_corpus, size=300, min_count=1,max_vocab_size=None,iter=100,
                              sorted_vocab=1)    

    fasttext.wv.save_word2vec_format('%s/Embedding/FastText/FastText.300d.txt'%(folder), binary=False)

#模型讀取方式
wvmodel = gensim.models.KeyedVectors.load_word2vec_format(
    '%s/Embedding/FastText/FastText.300d.txt'%(folder), binary=False, encoding='utf-8')
wvmodel.most_similar(u"player", topn=10)  
