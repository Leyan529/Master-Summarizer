...
class encoder_attention(nn.Module):
    ...
    def __init__(self):
        ...
        if config.hidden_dim == 768:
            self.W_h = nn.Linear(config.hidden_dim, config.hidden_dim, bias=False) # orign
        else:
            self.W_h = nn.Linear(config.emb_dim, config.hidden_dim, bias=False)
        self.W_s = nn.Linear(config.hidden_dim * 2, config.hidden_dim)
...
class Decoder(nn.Module):
    def __init__(self):
        ...
        if config.hidden_dim == 768:
            self.p_gen_linear = nn.Linear(config.hidden_dim * 4 + config.emb_dim, 1) # orign
        else:
            self.p_gen_linear = nn.Linear(config.hidden_dim * 3, 1) # new

        #p_vocab
        self.V = nn.Linear(config.hidden_dim*3, config.hidden_dim)
    ...
    def forward(self, x_t, s_t, enc_out, enc_padding_mask, ct_e, extra_zeros, enc_batch_extend_vocab, sum_temporal_srcs, prev_s, key_x, key_mask):
        ...
        # print('s_t',s_t.shape)
        # if len(s_t) > 0: s_t = s_t[0]
        if config.hidden_dim == 768:
            s_t = T.reshape(s_t,(s_t.shape[0],config.hidden_dim)) ; print('s_t',s_t.shape)
        else:
            s_t = T.rand(1,512).cuda() ; print('s_t',s_t.shape)
        # print('x',x.shape)
        s_t = self.lstm(x,(s_t,s_t)) # initialize the hidden state and cell state of LSTMCell 吐出 s_t     
        # print('s_t',s_t.shape)
        ...
        # print('p_gen',p_gen.shape);print('p_gen_linear',config.hidden_dim * 3 + config.emb_dim*2, 1)
        ...
        # print(prev_s.shape)
        # print('--------------------------------------------------------------------------')
        return final_dist, s_t[0], ct_e, sum_temporal_srcs, prev_s
